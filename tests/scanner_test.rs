...
"#).unwrap();
    ...
    dir.child("src/domain/Money.ts")
        .write_str("export record Money(amount: number, currency: string) {}").unwrap();
...
"#).unwrap();
    ...
    dir.child("src/infra/Database.ts")
        .write_str("export class Database { connect() {} }").unwrap();
...
fn detects_layers_in_layered_project() {
    ...
    assert!(model.layers.iter().any(|l| l == "controllers"), "should detect controllers");
    assert!(model.layers.iter().any(|l| l == "services"),    "should detect services");
    assert!(model.layers.iter().any(|l| l == "domain"),      "should detect domain");
    assert!(model.layers.iter().any(|l| l == "repositories"),"should detect repositories");
}
...
fn domain_appears_last_in_layer_order() {
    ...
    if let (Some(d), Some(c)) = (domain_pos, ctrl_pos) {
        assert!(d > c, "domain should appear after controllers in layer order");
    }
}
...
fn handles_project_with_no_adrs() {
    ...
    dir.child("src/services/Foo.ts")
        .write_str("export class Foo {}").unwrap();
    let model = scanner::scan(dir.path()).unwrap();
